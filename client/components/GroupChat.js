...
export default function GroupChat({ socketRef, name, title = 'Group Chat', channel, roomId, className = '' }) {
  ...
  useEffect(() => { roomRef.current = roomId; channelRef.current = channel; }, [roomId, channel]);
  ...
  useEffect(() => {
    ...
    const checkSocket = setInterval(() => {
      if (socketRef.current) {
        ...
        if (roomId && name) socketRef.current.emit('join-room', { roomId, name });
  ...
  useEffect(() => {
    try {
      const key = getStorageKey();
      if (typeof window !== 'undefined' && key) localStorage.setItem(key, JSON.stringify(messages));
    } catch {}
  }, [messages]);
  ...
  const getStorageKey = () => `${STORAGE.CHAT_PREFIX}${channelRef.current || 'solo'}:${roomRef.current || 'local'}`;
    ...
    <aside className={`h-full flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 border-l-4 border-transparent relative overflow-hidden ${className}`} style={{ borderImage: 'linear-gradient(180deg, #3b82f6, #8b5cf6, #ec4899) 1' }}>
      ...
      <div className="relative px-5 py-4 bg-gradient-to-r from-white/80 to-blue-50/80 backdrop-blur-lg border-b-2 border-transparent" style={{ backgroundImage: 'linear-gradient(white, white), linear-gradient(90deg, #3b82f6, #8b5cf6)', backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' }}>
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">💬 {title}</p>
          <span className="ml-auto text-xs font-semibold text-slate-400 bg-slate-100 px-2 py-1 rounded-full">{messages.length}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 relative z-10 text-slate-800">
        {messages.map((m, idx) => (
          <div key={idx} className="text-sm break-words leading-6">
            ...
            <span className={`ml-1 ${m.user === '' ? 'text-green-300 italic font-bold' : 'text-slate-800'}`}>{m.text}</span>
          </div>
        ))}
      ...
      <form onSubmit={sendMessage} className="relative p-4 bg-white/90 backdrop-blur-lg border-t-2 border-transparent z-10" style={{ backgroundImage: 'linear-gradient(white, white), linear-gradient(90deg, #3b82f6, #8b5cf6)', backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' }}>
        <div className="flex gap-2">
          <input className="flex-1 border-2 border-transparent rounded-xl px-4 py-3 outline-none transition-all duration-300 font-medium text-slate-700 placeholder:text-slate-400" placeholder="💭 Type your message..." value={input} onChange={(e) => setInput(e.target.value)} style={{ backgroundImage: 'linear-gradient(white, white), linear-gradient(135deg, #3b82f6, #8b5cf6)', backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box', border: '2px solid transparent' }} />
          <button type="submit" className="relative px-5 py-3 rounded-xl font-bold bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:from-blue-600 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl overflow-hidden group">
